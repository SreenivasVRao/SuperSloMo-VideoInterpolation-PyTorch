...
from .layers import avg_pool, conv, warp
...
class FlowInterpolationModel(nn.Module):
    ...
    def compute_inputs(self, img_tensor, flow_pred_tensor, t):
        ...
        input_tensor = torch.cat(
            [img_1, warped_img_1t, est_flow_t1, est_flow_t0, warped_img_0t, img_0],
            dim=1,
        )

        if self.verbose:
            log.info("Generated Input tensor of shape: %s", str(input_tensor.shape))
    ...
    def compute_output_image(self, img_tensor, input_tensor, output_tensor, t):
        ...
        pred_v_1t, pred_dflow_t1, pred_dflow_t0, pred_v_0t = self.extract_outputs(
            output_tensor
        )
