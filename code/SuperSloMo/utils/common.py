...
class ResizeCrop(object):
    ...
    def __call__(self, sample_frames):
        ...
        if new_imh < self.crop_imh or new_imw < self.crop_imw:
            ...
            print('self.crop_imh: {}, self.crop_imw: {}'.format(self.crop_imh, self.crop_imw))
            raise RuntimeError('Input images are too small.')

        for idx in range(sample_frames.shape[0]):
            new_frames[idx, ...] = cv2.resize(sample_frames[idx, ...], (new_imw, new_imh))
        h_start = np.random.randint(0, new_imh - self.crop_imh + 1)
        w_start = np.random.randint(0, new_imw - self.crop_imw + 1)
        new_frames = new_frames[:, h_start:h_start + self.crop_imh, w_start:w_start + self.crop_imw, ...]
...
class Normalize(object):
    ...
    def __call__(self, sample_tensor):
        # _, _, _, C = sample_tensor.shape
        # print(sample_tensor.shape)
        # assert C == 3, 'RGB input is required. Got {} channels.'.format(C)
        # for idx in range(C):
        #     sample_tensor[:, idx, ...] = \
        #         (sample_tensor[:, idx] / self.divisor - self.pix_mean[idx]) / self.pix_std[idx]
        sample_tensor = (sample_tensor / self.divisor - self.pix_mean) / self.pix_std
        return sample_tensor
